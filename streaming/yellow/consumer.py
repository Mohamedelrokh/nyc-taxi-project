...
def insert_taxi_trip(cursor, message_key, tpep_pickup_datetime, tpep_dropoff_datetime, pulocation_id, dolocation_id, passenger_count, trip_distance, fare_amount):
    ...
    try:
        insert_query = """
            INSERT INTO yellow_taxi_trips (message_key, tpep_pickup_datetime, tpep_dropoff_datetime, pulocation_id, dolocation_id, passenger_count, trip_distance,payment_type, fare_amount)
...
def main():
    config = read_config()
    topic = "yellow-trips"  # Kafka topic name (same as the producer's topic)
